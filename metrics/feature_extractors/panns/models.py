...
class Cnn14(nn.Module):
    def __init__(
        ...
        def ensure_checkpoint_exists(home_dir, checkpoint_name, url):
            checkpoint_path = f"{home_dir}/.cache/audioldm_eval/ckpt/{checkpoint_name}"
            if not os.path.exists(checkpoint_path):
                print(f"Downloading {checkpoint_name}")
                os.makedirs(f"{home_dir}/.cache/audioldm_eval/ckpt", exist_ok=True)
        ...
        if sample_rate == 16000:
            ...
            checkpoint_path = ensure_checkpoint_exists(home_dir, checkpoint_name, url)
            state_dict = torch.load(checkpoint_path)
            self.load_state_dict(state_dict["model"])
        elif sample_rate == 32000:
            checkpoint_name = "Cnn14_mAP=0.431.pth"
            url = "https://zenodo.org/record/3576403/files/Cnn14_mAP%3D0.431.pth"
            checkpoint_path = ensure_checkpoint_exists(home_dir, checkpoint_name, url)
            state_dict = torch.load(checkpoint_path)
            self.load_state_dict(state_dict["model"])
